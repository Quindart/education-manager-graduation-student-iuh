...
const useExportExcel = () => {
  ...
  const onExport = (
    ...
    switch (entity) {
      case 'topic':
        const TopicExport = new TopicClassExportExcel(fileName, sheetName, headerColumn);
        TopicExport.setData(data);
        TopicExport.customizeSheet();
        TopicExport.onExport();
        break;
      case 'lecturer':
        const LecturerExport = new LecturerClassExportExcel(fileName, sheetName, headerColumn);
        LecturerExport.setData(data);
        LecturerExport.customizeSheet();
        LecturerExport.onExport();
        break;
      case 'student':
        const StudentExport = new StudentClassExportExcel(fileName, sheetName, headerColumn);
        StudentExport.setData(data);
        StudentExport.customizeSheet();
        StudentExport.onExport();
        break;
      case 'groupStudent':
        const GrStudentExport = new GroupStudentClassExportExcel(fileName, sheetName, headerColumn);
        GrStudentExport.setData(data);
        GrStudentExport.customizeSheet();
        GrStudentExport.onExport();
        break;
      case 'assignGroup':
        const GrAssignExport = new AssginGroupClassExport(fileName, sheetName, headerColumn);
        GrAssignExport.setData(data);
        GrAssignExport.customizeSheet();
        GrAssignExport.onExport();
        break;
      case 'transcript':
        const TranscriptExport = new TranscriptExcel(fileName, sheetName, headerColumn);
        TranscriptExport.setData(data);
        TranscriptExport.customizeSheet();
        TranscriptExport.onExport();
        break;
      case 'demoScoreStudents':
        const DemoScoreExport = new DemoScoreStudentClassExport(fileName, sheetName, headerColumn);
      ...
      case 'allTotalTranscripts':
        const totalAllTranscripts = new TotalAllTranscripts(fileName, sheetName, headerColumn);
