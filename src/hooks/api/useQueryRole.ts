...
import {
  assignRoleToLecturer,
  getAllRoleLecturer,
  getRoleDetailByLecturerId,
  RoleBodyRequest,
  unAssignRoleToLecturer,
...
export const useRoleManager = () => {
  ...
  const handleGetRoleDetailByLecturerId = (lecturerId: string) => {
    return useQuery(
      [QueryKeysLecturer.getRoleDetailByLecturerId, lecturerId],
      () => getRoleDetailByLecturerId(lecturerId),
      {
        enabled: !!lecturerId,
        refetchOnMount: true,
      },
  ...
  const onAssignRoleToLecturer = (lecturerId: string) => {
    return useMutation((data: RoleBodyRequest) => assignRoleToLecturer(data), {
      onSuccess() {
        ...
        queryClient.invalidateQueries(QueryKeysLecturer.getAllRoleLecturer);
        queryClient.invalidateQueries(QueryKeysLecturer.getRoleDetailByLecturerId);
      },
      onError(err: any) {
