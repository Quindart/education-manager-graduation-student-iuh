...
function useArticle() {
  ...
  const handleGetAllArticle = () => {
    if (currentRoleRender === EnumRole.LECTURER) {
      return useQuery(
        QueryKeysArticle.GET_BY_LECTURER,
        () => ArticleService.getArticlesByLecturer(termId),
        {
          refetchOnMount: true,
          enabled: !!termId,
        },
  ...
  const onUpdateStatusArticle = (id: string) => {
    return useMutation(
      (data: { status; comment; bonusScore }) =>
        ArticleService.updateStatusArticle(id, data.status, data.comment, data.bonusScore),
      {
        onSuccess: (data) => {
