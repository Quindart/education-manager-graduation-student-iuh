...
import {
  addMemberToGroupLecturerById,
  removeMemberFromGroupLecturerById,
} from '@/services/apiGroupLecturer';
    ...
    return useMutation((lecturerId: string) => addMemberToGroupLecturerById(id, lecturerId), {
      onSuccess: (data: any) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeysGroupLecturer.getGroupLecturerById, id],
        });
        queryClient.invalidateQueries({
  ...
  const onRemoveMemberFromGroupLecturer = (id: string) => {
    return useMutation((data: any) => removeMemberFromGroupLecturerById(id, data), {
      onSuccess: (data: any) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeysGroupLecturer.getAllGroupLecturerByTypeGroup, 'reviewer', termId],
        });
      },
      onError(err: any) {
        if (err.status < 500) enqueueSnackbar(err.message, { variant: 'error' });
