...
export const useLecturer = () => {
  ...
  const handleGetCountOfMajorLecturer = () => {
    return useQuery(
      [QueryKeysLecturer.getCountOfMajorLecturer, majorId],
  ...
  const handleGetLecturerToExport = () => {
    return useQuery(
      [QueryKeysLecturer.getLecturerToExport, majorId],
      () => LecturerServices.getLecturerToExport(majorId),
      {
        enabled: !!majorId,
      },
    );
  ...
  const handleGetLecturerById = (id: string) => {
    return useQuery(
      [QueryKeysLecturer.getLecturerById, id],
      () => LecturerServices.getLecturerById(id),
      {
        enabled: !!id,
        cacheTime: 1000 * (60 * 1),
      },
  ...
  const onCreateLecturer = () => {
    return useMutation((lecturer: Partial<Lecturer>) => LecturerServices.createLecturer(lecturer), {
      onSuccess() {
        ...
        queryClient.invalidateQueries([QueryKeysLecturerTerm.getAllLectuerTermByParams, termId]);
        queryClient.invalidateQueries({
          queryKey: [QueryKeysLecturerTerm.listLecturerTerms, termId],
        });
      },
      onError(err: Pick<ResponseType, 'status' | 'message'>) {
        if (err.status < 500) {
          enqueueSnackbar(err.message, { variant: 'error' });
        } else enqueueSnackbar('Thêm giảng vien thất bại', { variant: 'error' });
      },
    });
  };
  ...
  const onResetPassword = () => {
    return useMutation((lecturerId: string) => LecturerServices.resetPassword(lecturerId), {
      ...
      onError(err: Pick<ResponseType, 'status' | 'message'>) {
        if (err.status < 500) {
          enqueueSnackbar(err.message, { variant: 'error' });
        } else enqueueSnackbar('Cập nhật mật khẩu giảng vien thất bại', { variant: 'error' });
      },
    });
  };
  ...
  const onUpdateLecturer = (id?: string) => {
    ...
    return useMutation(
      (lecturer: Lecturer) => LecturerServices.updateLecturerById(lecturerId, lecturer),
      {
        onSuccess() {
          ...
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupLecturer.getAllGroupLecturerByTypeGroup, 'reviewer'],
          });
          queryClient.invalidateQueries([
