...
const useGroupStudent = () => {
  ...
  const hanldeSearchGroupStudents = (name: string) => {
    return useQuery([QueryKeysGroupStudent.searchGroupStudentByName, termId, name], () =>
      GroupStudentServices.searchGroupStudentAdmin(termId),
    );
  };
  const handleManagerRenderActionGroupStudent = () => {
    return useQuery(
      [QueryKeysGroupStudent.managerActionGroupStudent, termId],
      () => GroupStudentServices.searchGroupStudentAdmin(termId),
      {
  ...
  const handleGetGroupStudentByLecturerByTerm = (lecturerId: string) => {
    return useQuery(
      [QueryKeysGroupStudent.getGroupStudentByLecturerSupport, termId, lecturerId],
      () => GroupStudentServices.getGroupStudentByLecturerByTerm(termId, lecturerId),
      {
        staleTime: 10000,
        refetchOnMount: true,
      },
  ...
  const onUnAssignTopicGroupStudent = (topicId: string) => {
    return useMutation((id: string) => GroupStudentServices.removeAssign(id, topicId), {
      onSuccess(data: any) {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeysGroupStudent.getStudentsNoHaveGroup, termId],
        });
  ...
  const handleGetExportGroupStudent = () => {
    return useQuery([QueryKeysGroupStudent.getExportGroupStudent, termId], () =>
      GroupStudentServices.getExportGroupStudent(termId),
    );
  };
  const handleGetCountOfGroupStudent = () => {
    return useQuery([QueryKeysGroupStudent.getCountOfGroupStudent, termId], () =>
      GroupStudentServices.getCountOfGroupStudent(termId),
    );
  };
  //[GET BY ID]
  const handleGetGroupStudentById = (id: string) => {
    return useQuery(
      [QueryKeysGroupStudent.getGroupStudentById, id],
      () => GroupStudentServices.getGroupStudentById(id),
      {
        staleTime: 1000,
        refetchOnMount: true,
        enabled: !!id,
  ...
  const handleGetStudentNoHaveGroup = () => {
    return useQuery(
      [QueryKeysGroupStudent.getStudentsNoHaveGroup, termId],
      () => getStudentsNoHaveGroup(termId),
      {
        refetchOnMount: true,
      },
    );
  ...
  const onCreateGroupStudent = () => {
    return useMutation(
      (studentIds: string[]) =>
        GroupStudentServices.createGroupStudent({ termId: termId, studentIds }),
      {
        onSuccess(data: any) {
          ...
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupStudent.getCountOfGroupStudent, termId],
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupStudent.getStudentsNoHaveGroup, termId],
          });
        ...
        onError(err: any) {
          ...
          } else enqueueSnackbar('Cập nhật thất bại', { variant: 'error' });
        },
      },
    );
  };
