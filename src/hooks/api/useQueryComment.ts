...
function useComment() {
    ...
    const handleGetComment = (type: string, groupStudentId: string) => {
        return useQuery([QueryKeysComment.GET_COMMENT_BY_GROUP_STUDENT, type, termId, groupStudentId], () => CommentService.getCommentById(termId, type, groupStudentId), {
            refetchOnMount: true
        })
    }
    const onCreateComment = () => {
        return useMutation((data: { type: string, groupStudentId: string, content: string }) => CommentService.createComment(termId, data.type, data.groupStudentId, data.content), {
            onSuccess: (data) => {
    ...
    const onUpdateComment = () => {
        return useMutation((data: { id: string, content: string }) => CommentService.updateComment(data.id, data.content), {
            onSuccess: (data) => {
                queryClient.invalidateQueries(QueryKeysComment.GET_COMMENT_BY_GROUP_STUDENT)
                if (data.success === true) {
