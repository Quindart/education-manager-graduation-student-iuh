...
import {
  addMemberInGroup,
  deleteMemberInGroup,
  getMemberInGroupStudent,
} from '@/services/apiGroupStudent';
...
const useMemberGroupStudent = () => {
  ...
  const handleGetMemberInGroupStudent = (id: string) => {
    return useQuery(
      [QueryKeysMemberOfGroupStudent.getMemberInGroupStudent, id],
      () => getMemberInGroupStudent(id),
      {
        enabled: !!id,
      },
    );
  ...
  const onUpdateStatusStudentMember = (id: string, studentId: string) => {
    return useMutation(
      (data: { status: string; termId: string }) => updateStatusStudent(studentId, data),
      {
        onSuccess(data: any) {
          if (data.success)
            enqueueSnackbar('Cập nhật trạng thái sinh viên thành công', { variant: 'success' });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysMemberOfGroupStudent.getMemberInGroupStudent, id],
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupStudent.getGroupStudentById, id],
          });
        },
        onError(err: any) {
  ...
  const onDeleteStudentMember = (id: string) => {
    return useMutation((data: any) => deleteMemberInGroup(id, data), {
      onSuccess() {
        ...
        queryClient.invalidateQueries([
          QueryKeysGroupStudent.getStudentsNoHaveGroup,
          termStore.currentTerm.id,
        ]);
        queryClient.invalidateQueries([
          QueryKeysGroupStudent.getCountOfGroupStudent,
          termStore.currentTerm.id,
        ]);
        queryClient.invalidateQueries([
        ...
        queryClient.invalidateQueries([
          QueryStudent.getSearchStudentBasic,
          termStore.currentTerm.id,
          '',
          '',
        ]);
  ...
  const onAddStudentMember = (id: string) => {
    return useMutation(
      (data: { studentId: string; termId?: string }) =>
        addMemberInGroup(id, { ...data, termId: termStore.currentTerm.id }),
      {
        onSuccess() {
  ...
  return {
    ...
    onDeleteStudentMember,
    onAddStudentMember,
    onUpdateStatusStudentMember,
  };
};
