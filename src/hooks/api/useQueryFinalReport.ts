...
function useFinalReport() {
  ...
  const handlegetAllFinalReport = () => {
    if (currentRoleRender === EnumRole.LECTURER) {
      return useQuery(
        [QueryKeysFinalReport.getFinalReportByLecturer, termId],
        () => FinalReportService.getFinalReportsByLecturerId(termId),
        {
          refetchOnMount: true,
          enabled: !!termId,
        },
    ...
    return useQuery(
      [QueryKeysFinalReport.getAllFinalReport, termId],
      () => FinalReportService.getFinalReports(termId),
      {
        refetchOnMount: true,
        enabled: !!termId,
      },
  ...
  const handlegetFinalReportById = (id: string) => {
    return useQuery(
      QueryKeysFinalReport.getFinalReportById,
      () => FinalReportService.getFinalReportById(id),
      {
        refetchOnMount: true,
        enabled: !!id,
      },
  ...
  const onCommentFinalReport = (id: string) => {
    return useMutation(
      (comment: string) => FinalReportService.updateCommentFinalReport(id, comment),
      {
        onSuccess: (data) => {
