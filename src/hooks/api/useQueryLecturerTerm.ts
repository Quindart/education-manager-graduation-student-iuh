...
export const useLecturerTerm = () => {
  ...
  const handleGetExportLecturerTermAssign = () => {
    return useQuery([QueryKeysLecturerTerm.getExportAssignLecturerTerm, termId], () =>
      LecturerTermServices.getExportAssignLecturerTerm(termId),
    );
  };
  // [GET ALL]
  ...
  const handleGetLecturerTermById = (id: string) => {
    return useQuery(
      [QueryKeysLecturerTerm.getLecturerTermById, id],
      () => LecturerTermServices.getLecturerTermById(id),
      {
        enabled: !!id,
      },
    );
  ...
  const handleGetListLecturerTerms = (keyword: string) => {
    return useQuery(
      [QueryKeysLecturerTerm.listLecturerTerms, termId, keyword],
      () => LecturerTermServices.getListLecturerTerm(termId, keyword),
      {
        refetchOnMount: true,
      },
    );
    ...
    return useMutation((id: string) => LecturerTermServices.deleteLecturerTermById(id), {
      onSuccess: (data: Pick<ResponseType, 'success' | 'message'>) => {
        if (data.success === true) {
          ...
          queryClient.invalidateQueries([
            QueryKeysLecturerTerm.lecturerTermsToAdd,
            termId,
            majorId,
          ]);
  ...
  const handleLecturerTermsToAdd = () => {
    return useQuery(
      [QueryKeysLecturerTerm.lecturerTermsToAdd, termId, majorId],
      () => LecturerTermServices.getListLecturerTermToAdding(termId, majorId),
      {
  ...
  const onCreateLecturerTerm = () => {
    return useMutation(
      (data: { lecturerId: string; termId: string }) =>
        LecturerTermServices.createLecturerTerm(data),
      {
        onSuccess(data: any) {
          if (data.success === true) {
            enqueueSnackbar(data.message, { variant: 'success' });
  ...
  const onImportLecturerTerm = () => {
    return useMutation(
      () => LecturerTermServices.importLecturerTerm(termId, majorStore.currentMajor.id),
      {
