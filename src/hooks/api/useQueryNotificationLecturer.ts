...
export const useNotificationLecturer = () => {
  ...
  const handleGetMyNotification = (limit: string) => {
    return useQuery(
      [QueryKeysNotificationLecturer.getMyNotification, limit],
      () => NotificationLecturerServices.getMyNotification(limit),
      {},
    );
  };
  //[GET]
  const handleGetNotificationOfLecturer = (id: string) => {
    return useQuery([QueryKeysNotificationLecturer.getNotificationsOfLecturer, id], () =>
      NotificationLecturerServices.getNotificationsOfLecturer(id),
    );
  };
  //[POST]
  const onCreateNotificationOfLecturerIds = () => {
    return useMutation(
      (data: { title: string; content: string; lecturerIds: string[] }) =>
        NotificationLecturerServices.createNotificationOfLecturerIds(data),
      {
        onSuccess(data: any) {
  ...
  const onCreateAllNotificationLecturerTerms = () => {
    return useMutation(
      (data: { title: string; content: string }) =>
        NotificationLecturerServices.createAllNotificationLecturerTerms(
          data.title,
          data.content,
          termId,
        ),
      {
        onSuccess(data: any) {
          enqueueSnackbar('Gửi thông báo tới toàn bộ giảng viên thành công !', {
            variant: 'success',
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysNotification.getNotificationsOfFilter, '10', '1', '', ''],
  ...
  const onCreateNotificationOfGroupLecturer = () => {
    return useMutation(
      (data: { title: string; content: string; groupLecturerIds: string[] }) =>
        NotificationLecturerServices.createNotificationOfGroupLecturer(data),
      {
        onSuccess(data: any) {
          enqueueSnackbar('Gửi thông báo thành công', { variant: 'success' });
          queryClient.invalidateQueries({
  ...
  const onUpdateReadStatus = (id: string) => {
    return useMutation(() => NotificationLecturerServices.upateReadStatusNotification(id), {
      onSuccess(data: any) {
        ...
        queryClient.invalidateQueries([
          QueryKeysNotificationLecturer.getNotificationsOfLecturer,
          id,
        ]);
        queryClient.invalidateQueries([QueryKeysNotificationLecturer.getMyNotification]);
      },
