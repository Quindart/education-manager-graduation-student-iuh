...
export const useNotification = () => {
  ...
  const getRolePermission = () => {
    ...
    if (
      currentRoleRender === RoleCheck.ADMIN ||
      currentRoleRender === RoleCheck.HEAD_COURSE ||
      currentRoleRender === RoleCheck.HEAD_LECTURER
    ) {
  ...
  const handleGetNotificationById = (id: string) => {
    return useQuery(
      [QueryKeysNotification.getNotificationById, id],
      () => NotificationServices.getNotificationById(id),
      {
  ...
  const onCreateManyNotifications = () => {
    return useMutation(
      (data: any) => NotificationServices.createNotifications(data.title, data.content, termId),
      {
        onSuccess: (data: any) => {
          if (data.success) {
            enqueueSnackbar('Thêm thông báo thành công', { variant: 'success' });
            queryClient.invalidateQueries(QueryKeysNotification.getNotificationsOfFilter);
  ...
  const onUpdateNotification = (id: string) => {
    return useMutation(
      (data: { title: string; content: string }) =>
        NotificationServices.updateNotification(id, data.title, data.content),
      {
        onSuccess: (data: any) => {
          if (data.success) {
