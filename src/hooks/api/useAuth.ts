...
import {
  setCurrentRoleRender,
  setMe,
  setParamTotalPageLectuerMajor,
  setParamTotalPageLectuerTerm,
} from '@/store/slice/lecturer.slice';
...
export const useAuth = () => {
  ...
  //[LOGIN]
  const handleLogin = () => {
    return useMutation((data: IAuth) => login(data), {
  ...
  const onUpdatePassword = () => {
    return useMutation((data: { password: string; newPassword: string }) => updatePassword(data), {
      onSuccess: () => {
      ...
      onError: (error: any) => {
        if (error.status >= 500)
          enqueueSnackbar('Hệ thống không thể xử lý yêu cầu của bạn. Thử lại sau', {
            variant: 'warning',
          });
  ...
  const onUpdateMe = () => {
    return useMutation(
      (data: { fullName: string; email: string; phone: string; gender: string }) => updateMe(data),
      {
        onSuccess: () => {
