...
import {
  EvaluationDataRequestType,
  TypeEvaluation,
  createEvaluation,
  deleteEvaluation,
  getEvaluationById,
  getEvaluationByTermByType,
  updateEvaluation,
...
import {
  EvaluationLecturer,
  GroupLecturerServiceKeys,
} from '@/page/ReviewManager/Entity/EvaluationManager';
...
const useEvaluation = () => {
  ...
  const handleGetEvalutationByType = (termId?: string, type?: TypeEvaluation) => {
    return useQuery(
      [QueryEvaluation.getEvaluationByType, termId, type],
  ...
  const handleGetEvaluationById = (evaluationId?: string) => {
    return useQuery(
      [QueryEvaluation.getEvaluationById, evaluationId],
    ...
    return useMutation((data: EvaluationDataRequestType) => createEvaluation(data), {
      onSuccess() {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryEvaluation.getEvaluationByType, termId, type],
        });
      },
      onError(err: any) {
  ...
  const onUpdateEvaluationById = (
    ...
    return useMutation((data: EvaluationDataRequestType) => updateEvaluation(evaluationId, data), {
      onSuccess() {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryEvaluation.getEvaluationById, evaluationId],
        });
      },
      onError(err: any) {
        if (err.status < 500) enqueueSnackbar(err.message, { variant: 'error' });
