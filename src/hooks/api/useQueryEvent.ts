...
function useEvent() {
  ...
  const onUpdateEventById = (id: string) => {
    return useMutation(
      (data: Partial<EventType>) =>
        EventServices.updateEvent(id, data.name, data.startDate, data.endDate),
      {
        onSuccess() {
  ...
  const onCommentEvent = (id: string) => {
    return useMutation(
      (data: any) => EventServices.updateCommentEventById(id, data.groupStudentId, data.comment),
      {
        onSuccess() {
