...
function useGroupSupport() {
  ...
  const handleGetMyGroupSupport = () => {
    return useQuery([QuertKeysGroupSupport.getMyGroupSupport, termStore.currentTerm.id], () =>
      getMyGroupSupport(termStore.currentTerm.id),
    );
  };
  const handleGetStudentMemberToScoring = (termId?: string) => {
    return useQuery(
      [QuertKeysGroupSupport.getStudentMemberToScoring, termStore.currentTerm.id],
      () => getStudentMemberToScoring(termStore.currentTerm.id),
    );
  };
  const handleGetGrToExport = () => {
    return useQuery([QuertKeysGroupSupport.getGrStdToExport, termStore.currentTerm.id], () =>
      getGroupStdsExportByLecturer(termStore.currentTerm.id),
    );
  };
  return {
    handleGetGrToExport,
