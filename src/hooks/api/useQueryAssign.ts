...
const useAssign = () => {
  ...
  const handletGetGroupStudentNoAssignByType = (type: string) => {
    return useQuery(
      [QueryKeysAssign.getGroupStudentNoAssignByType, type, termId],
      () => AssignServices.getGroupStudentNoAssign(type, termId),
      {
        refetchOnMount: true,
        enabled: !!termId,
      },
  ...
  const onCreateAssignByType = (type: string, groupLecturerId: string) => {
    return useMutation(
      (data: { groupLecturerId: string; listGroupStudentId: string[]; type: string }) =>
        AssignServices.createAssignByType(data),
      {
        onSuccess() {
          ...
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupLecturer.getGroupLecturerById, groupLecturerId],
          });
  ...
  const onUpdateAssignByType = (type: string, groupLecturerId: string) => {
    return useMutation(
      (data: { groupLecturerId: string; listGroupStudentId: string[]; type: string }) =>
        AssignServices.updateAssignByType(data),
      {
        onSuccess() {
          enqueueSnackbar('Cập nhật phân công thành công', { variant: 'success' });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysAssign.getGroupStudentNoAssignByType, type, termId],
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupLecturer.getGroupLecturerById, groupLecturerId],
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysGroupLecturer.getAllGroupLecturerByTypeGroup, type, termId],
          });
          queryClient.invalidateQueries({
            queryKey: [QueryKeysAssign.getExportAssignGroup, termId, type],
          });
        },
        onError(err: any) {
          enqueueSnackbar(err.message, { variant: 'error' });
  ...
  const handleGetExportAssignGroup = (type: string) => {
    ...
    return useQuery(
      [QueryKeysAssign.getExportAssignGroup, termId, typeSend],
      () => AssignServices.getExportAssignGroup(termId, typeSend),
      {
        refetchOnMount: true,
        enabled: !!typeSend,
      },
  ...
  const handleExportExcelAssignByLecturerId = (type: string) => {
    let typeSend = type === 'reviewer' ? type : 'report';
    return useQuery(
      [QueryKeysAssign.getExportAssignGroup, termId, type],
      () => AssignServices.getExportAssignGrByLecturerId(termId, typeSend),
      {
        refetchOnMount: true,
        enabled: !!termId,
      },
