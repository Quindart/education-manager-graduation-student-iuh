...
export const useTopic = () => {
  ...
  const handleGetCountOfTopic = () => {
    return useQuery([QueryTopic.getCountOfTopic, termId], () => TopicServices.getCountOfTopic(termId));
  };
  ...
  const handleUiRender = (): string[] => {
    ...
    if (
      currentRole === RoleCheck.HEAD_LECTURER ||
  ...
  const handleGetKeywordsOfTopics = () => {
    return useQuery(
      [QueryTopic.getKeywordsOfTopic, termId],
      () => TopicServices.getKeywords(termId),
      {
        enabled: !!termId,
      },
    );
  ...
  const hanldeGetGroupsByTopic = (topicId: string) => {
    return useQuery(
      [QueryTopic.getGroupByTopic, termStore.currentTerm.id, topicId],
      () => getGroupByTopic(termStore.currentTerm.id, topicId),
      {
        enabled: !!topicId,
      },
    );
  ...
  const handleGetTopicToExport = () => {
    return useQuery(
      [QueryTopic.getTopicToExport, termId],
      () => TopicServices.getTopicsExport(termId),
      {
        enabled: !!termId,
      },
    );
  ...
  const handleGetTopicLecturerToExport = () => {
    return useQuery(
      [QueryTopic.getTopicLecturerToExport, termId],
      () => TopicServices.getTopicsExportByLecturer(termId),
      {
        enabled: !!termId,
      },
    );
  ...
  const handleTopicsByTermByMajor = () => {
    return useQuery(
      [QueryTopic.getSearchTopic, termStore.currentTerm.id, ''],
      () => TopicServices.getTopicsByTermByMajor(termStore.currentTerm.id),
      {
        staleTime: 1000 * (60 * 3), // 10 min,
      },
    );
  ...
  const handleTopicsByMe = () => {
    return useQuery(
      [QueryTopic.getTopicsByMe, termStore.currentTerm.id, lecturerStore.me.user.id],
      () =>
        TopicServices.getTopicsByLecturerByTerm(lecturerStore.me.user.id, termStore.currentTerm.id),
      {
        staleTime: Infinity,
        onSuccess(data) { },
      },
  ...
  const handleGetTopicsByGroupLecturerAssigned = (groupId: string) => {
    return useQuery(
      [QueryTopic.getTopicsByGroupLecturerAssigned, groupId],
      () => TopicServices.getTopicsByGroupLecturerAssigned(groupId),
      {
        enabled: !!groupId,
      },
    );
  ...
  const handleTopicsByLecturerByTerm = (lecturerId: string) => {
    return useQuery(
      [QueryTopic.getTopicsByLecturerTermId, termStore.currentTerm.id, lecturerId],
      () => TopicServices.getTopicsByLecturerByTerm(lecturerId, termStore.currentTerm.id),
      {
        staleTime: Infinity,
        onSuccess(data) { },
      },
  ...
  const onCreateTopicByToken = () => {
    return useMutation(
      (newTopic: TopicBodyRequestType) =>
        TopicServices.createTopicByToken(newTopic, termStore.currentTerm.id),
      {
        onSuccess() {
  ...
  const onUpdateStatusTopic = (topicId: string) => {
    return useMutation(
      (data: Pick<Topic, 'status' | 'note'>) => TopicServices.updateStatusTopicById(topicId, data),
      {
        onSuccess() {
          enqueueSnackbar(MESSAGE_STORE_SUCCESS(TypeMess.update, 'Đề tài'), { variant: 'success' });
          queryClient.invalidateQueries(QueryTopic.getSearchTopic);
  ...
  const onUpdateAllQuantityGroupMax = () => {
    return useMutation(
      (quantity: number) =>
        TopicServices.updateAllQuantityGroupMax(termStore.currentTerm.id, quantity),
      {
        onSuccess() {
          enqueueSnackbar(MESSAGE_STORE_SUCCESS(TypeMess.update, 'Đề tài'), { variant: 'success' });
          queryClient.invalidateQueries(QueryTopic.getSearchTopic);
