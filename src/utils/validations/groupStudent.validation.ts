...
export const isPassStatus = (status: string) => {
  return (
    status === EnumStatusStudent.OPEN ||
    status === EnumStatusStudent.PASS_ADVISOR ||
    status === EnumStatusStudent.PASS_REVIEWER ||
    status === EnumStatusStudent.PASS_REPORT
  );
};
...
export const getStatusStudentStyle = (statusId: string) =>
  STATUS_STYLES[statusId] || { color: '#000', backgroundColor: '#FFF' };
