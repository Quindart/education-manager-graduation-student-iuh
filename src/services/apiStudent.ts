...
export const getStatisticStatusOfStudents = async (
  termId: string,
  status: EnumStatusStudent,
...
//[GET]
export const getSearchStudentBasic: any = (
  termId: string,
  keywords: string,
  searchField: string,
) => {
  ...
  return axiosConfig.get(
    `${URL}/search?termId=${termId}&keywords=${keywordsSender}&searchField=${searchFieldSender}`,
  );
};
//[GET]
...
//[Admin role]
export const getStudentOfSearch: any = (
  termId: string,
  majorId: string,
  limit: number,
  page: number,
  searchField: 'full_name' | 'username' | 'phone' | 'email',
  sort: string,
  keywords: string,
) => {
  ...
  return axiosConfig.get<ResponseType, any>(
    `${URL}/query?searchField=${searchFieldSend}&keywords=${keywordSend}&limit=${limit}&page=${page}&termId=${termId}&majorId=${majorId}}&sort=${sortSend}`,
  );
};
//[GET]
...
//[GET]
export const getAllStudentByMajor = async (
  termId: string,
  majorId: string,
  limit: number,
  page: number,
  ...
  return axiosConfig.get<ResponseType, any>(
    `${URL}?limit=${limit}&page=${page}&termId=${termId}&majorId=${majorId}`,
  );
};
