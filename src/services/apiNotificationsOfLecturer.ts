...
export const createNotificationOfLecturerIds = (data: {
  title: string;
  content: string;
  lecturerIds: string[];
}) => {
...
export const createAllNotificationLecturerTerms = (
  title: string,
  content: string,
  termId: string,
) => {
  return axiosConfig.post<ResponseType, any>(`${URL}/terms?termId=${termId}`, {
    title: title,
    content: content,
    termId: termId,
  });
...
//[GROUP_LECTURER]
export const createNotificationOfGroupLecturer = (data: {
  title: string;
  content: string;
  groupLecturerIds: string[];
}) => {
