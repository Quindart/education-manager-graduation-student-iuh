...
//[Admin role]
export const getAllLecturerTermByParams = (
  termId: string,
  limit: number | string,
  page: number | string,
  searchField: string,
  ...
  return axiosConfig.get<ResponseType, any>(
    `${URL}/query?searchField=${searchFieldSend}&keywords=${keywordSend}&limit=${limit}&page=${page}&termId=${termId}&sort=${sortSend}`,
  );
};
...
export const getListLecturerTerm: any = async (termId: string, searchField: string) => {
  ...
  return axiosConfig.get<ResponseType, any>(
    `${URL}?termId=${termId}&searchField=${searchFieldSend}`,
  );
};
//[CREATE]
